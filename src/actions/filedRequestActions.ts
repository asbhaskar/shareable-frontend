...
export const addFiledRequest = async (
    organization: string,
    group: string,
    filedRequest: FiledRequest
): Promise<string> => {
    ...
    try {
        await setDoc(
            doc(
                firestore,
                `${ORGANIZATIONS_COLLECTION}/${organization}/${GROUPS_COLLECTION}/${group}/${REQUESTS_COLLECTION}`,
                filedRequestId
            ),
            filedRequest
        );
...
export const updateFiledRequest = (
    organization: string,
    group: string,
...
export const getFiledRequest = async (
    ...
    group: string,
    filedRequestId: string
): Promise<FiledRequest | undefined> => {
    try {
        ...
        if (filedRequestDocument.exists()) {
            ...
            console.log(
                'Error - FireStore - Getting FiledRequest: No filed request found for id: ' +
                    filedRequestId
            );
        }
    } catch (error: unknown) {
...
export const getFiledRequests = async (
    organization: string,
    group: string
): Promise<{ [id: string]: FiledRequest }> => {
