...
export const addInsight = async (organization: string, group: string, insight: Insight): Promise<string>  => {
...
export const getInsights = async (organization: string, group: string): Promise<{[id: string]: Insight}> => {
    const groupRef: CollectionReference =
        collection(firestore, `${ORGANIZATIONS_COLLECTION}/${organization}/${GROUPS_COLLECTION}/${group}/${INSIGHTS_COLLECTION}`);
